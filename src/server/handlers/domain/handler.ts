...
export class DomHandler {
    ...
    public get active(): any {
        return {
            getHandler: async (req: FastifyRequest<Request>, res: FastifyReply): Promise<void> => {

                const { user } = req.params;

                const active = await req.db.user.fetch(user as string);

                if (!active.success) return res.status(500).send({
            ...
            getPreHandler: async (req: FastifyRequest<Request>, res: FastifyReply): Promise<void> => {
                const { user } = req.params;
                ...
                if (!await req.db.user.exists(user)) return res.status(404).send({
                    status: 'USER_NOT_FOUND',
                    message: 'The user you are trying to view the active domain for does not exist',
                    code: 404
                });
            },
            setHandler: async (req: FastifyRequest<Request>, res: FastifyReply): Promise<void> => {
            ...
            setPreHandler: async (req: FastifyRequest<Request>, res: FastifyReply): Promise<void> => {
                ...
                if (!validateRequestBody) return res.status(400).send({
                    status: 'INVALID_REQUEST_BODY',
                    message: 'Please provide a valid request body with the domain and secret fields',
                    code: 400
                });

                if (!await req.db.user.exists(user as string)) return res.status(404).send({
                    status: 'USER_NOT_FOUND',
                    message: 'The user you are trying to activate the domain for does not exist',
                    code: 404
                });

                if (!(await req.db.secret.exists(secret))) return res.status(401).send({
                    status: 'INVALID_SECRET',
                    message: 'The secret you provided is invalid',
                    code: 401
                });

                if (!(await req.db.domain.exists({ domain }))) return res.status(404).send({
                    status: 'DOMAIN_NOT_FOUND',
                    message: 'The domain you are trying to activate does not exist',
                    code: 404
                });

                if (!(await req.db.domain.verifyRecord({ domain }))) return res.status(401).send({
                    status: 'DOMAIN_NOT_VERIFIED',
                    message: 'The domain you are trying to activate has not been verified',
                    code: 401
                })

                const valid = await req.db.domain.validate({ domain });

                if (!valid.success) return res.status(500).send({
...
function validateRequestBody(body: Request['Body'] | undefined): body is Request['Body'] {
